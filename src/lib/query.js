...
(function() {
	query.prototype = {
		...
		has: function(value) {
			if (this._expression)
				this._expression.template = "%not(%term != null && %term.indexOf(%value) != -1)";
			return this._oper(null, value);
		},
		startsWith: function(value) {
			if (this._expression)
				this._expression.template = "%not(%term != null && %term.substr(0, %operator) == %value)";
		...
		regex: function(value) {
			if (this._expression)
				this._expression.template = "%not(%term != null && %operator.test(%term))";
			return this._oper(this._store(value));
		},
		same: function(value) {
			if (this._expression)
				this._expression.template = "%not(JSON.stringify(%term) %operator %value)";
				...
				switch (match) {
					...
					case "%term":
						return (
							...
							'"] !== "undefined" ? obj["' +
							self.term +
							'"] : (typeof obj.get === "function" && obj.get("' +
							self.term +
							'")))' +
							(self.value instanceof Date ? ".getTime()" : "")
						);
					case "%operator":
